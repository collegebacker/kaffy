defmodule Kaffy.ResourceSchema do
  ...
  def kaffy_field_sortable?(_schema, {_field, options}),
    do: !Map.get(options || %{}, :no_sort, false)

  def kaffy_field_sortable?(_schema, _field_options), do: false

  def kaffy_target_url(entry, context, {_field, %{target_url: target_url}})
      when is_function(target_url) do
    target_url.(entry, context)
  end

  def kaffy_target_url(entry, _context, {_field, %{target_url: target_url}})
      when is_binary(target_url) do
